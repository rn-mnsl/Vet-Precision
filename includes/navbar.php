            ...
            <?php if (!empty($recent_notifications)): ?>
                <?php foreach ($recent_notifications as $note): ?>
                    <div class="notification-item <?php echo $note['is_read'] ? '' : 'unread'; ?>">
                        <?php echo sanitize($note['message']); ?><br>
                        <small><?php echo date('M d, Y h:i A', strtotime($note['created_at'])); ?></small>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
...
notificationBtn.addEventListener('click', function(event) {
    ...
    fetch(siteUrl + '/client/notifications/fetch.php')
        .then(response => response.json())
        .then(data => {
            if (data.length) {
                notificationDropdown.innerHTML = data.map(n =>
                    `<div class="notification-item ${n.is_read == 0 ? 'unread' : ''}">${n.message}<br><small>${n.created_at}</small></div>`
                ).join('');
            } else {
            ...
            fetch(siteUrl + '/client/notifications/mark_read.php');
        });
});
